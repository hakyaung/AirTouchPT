...
try:
    ...
    print("[Model] legraise_model.pkl loaded. (3D World Landmarks)")
except: pass

if not squat_model and not pushup_model and not legraise_model:
...
def squat_feedback(lm_list, stage):
    ...
    if ls_y - rs_y > 0.07:  return "상체가 한쪽으로 기울었습니다. 중심을 잡으세요."
    if rs_y - ls_y > 0.07:  return "상체가 한쪽으로 기울었습니다. 중심을 잡으세요."
    return None
...
# =============================================================
EXERCISE_MAP = {
    "스쿼트": "Squat",   "푸시업": "Pushup",  "팔굽혀": "Pushup", 
    "풀업":   "Pullup",  "턱걸이": "Pullup", 
    "싯업":   "Situp",   "윗몸":   "Situp", 
    "점핑":   "JumpingJack", "점핑잭": "JumpingJack",
    "레그레이즈": "LegRaise", "레그": "LegRaise", "다리올리기": "LegRaise"
}
KOR_NAME_MAP = {
...
while cap.isOpened():
    ...
    if not PT_MODE:
        ...
        if pose_res.pose_landmarks and ex_sub_mode != "Waiting":
            ...
            if active_model:
                # 🚨 [핵심 수정] 레그레이즈는 3D 절대 좌표(world_landmarks)를 사용, 나머지는 기존 2D(landmarks) 사용
                if ex_sub_mode == "LegRaise" and pose_res.pose_world_landmarks:
                ...
                try:
                    ...
                    # 70% 이상 확신할 때만 작동 (이중 잠금 1단계)
                    if confidence > 0.70:
                        pred_lower = prediction.lower().replace(" ", "")
                        
                        # 물리적 각도 이중 잠금장치 (이중 잠금 2단계 - 화면 2D 좌표인 lm_list 활용)
                        ls, rs = [lm_list[11].x, lm_list[11].y], [lm_list[12].x, lm_list[12].y]
                        lh, rh = [lm_list[23].x, lm_list[23].y], [lm_list[24].x, lm_list[24].y]
                        la, ra = [lm_list[27].x, lm_list[27].y], [lm_list[28].x, lm_list[28].y]
                        left_angle = get_angle(ls, lh, la)
                        right_angle = get_angle(rs, rh, ra)
                        avg_hip_angle = (left_angle + right_angle) / 2

                        is_physically_down = True
                        is_physically_up = True

                        if ex_sub_mode == "LegRaise":
                            # 몸이 거의 일직선에 가까울 때만 Down 인정
                            is_physically_down = avg_hip_angle > 140  
                            # 다리가 위로 확실히 꺾였을 때만 Up 인정
            ...
            if prediction is None and ex_sub_mode == "Squat":
                hip, knee, ankle = [lm_list[23].x, lm_list[23].y], [lm_list[25].x, lm_list[25].y], [lm_list[27].x, lm_list[27].y]
